package com.cursojava.aulawebservices.resources;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import com.cursojava.aulawebservices.entities.Category;
import com.cursojava.aulawebservices.services.CategoryService;

// RESOURCE layer - Recurso WEB referente a entidade Category

// Annotation.  Recurso WEB implementando por controlador REST
@RestController
//Indica caminho do recurso
@RequestMapping(value = "/categories")
public class CategoryResource {

	@Autowired
	private CategoryService service;
	
//Indica que o metodo responde a requisicao tipo GET do HTTP
	@GetMapping
// ResponseEntity - tipo de retorno especifico do spring para retorno requisicao WEB
// Metodo que é um end-point para acessar todas as categorias
	public ResponseEntity<List<Category>> findAll() {
		
		List<Category> list = service.findAll();
		
// ResponseEntity.ok - resposta com sucesso do HTTP, com corpo do objeto u
		return ResponseEntity.ok().body(list);
	}

// Metodo que é um end-point para acessar a categoria por id
	@GetMapping(value = "/{id}")
	public ResponseEntity<Category> findbyId(@PathVariable Long id) {
		Category obj = service.findById(id);

		// ResponseEntity.ok - resposta com sucesso do HTTP, com corpo do objeto u
		return ResponseEntity.ok().body(obj);
	}
}